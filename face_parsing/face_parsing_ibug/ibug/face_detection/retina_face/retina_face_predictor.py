...
class RetinaFacePredictor(object):
    ...
    def get_model(name: str = 'resnet50') -> SimpleNamespace:
        ...
        if name == 'resnet50':
            return SimpleNamespace(weights=os.path.realpath(os.path.join('pretrain_models/face_lib/face_parsing/ibug', 'Resnet50_Final.pth')),
                                   config=SimpleNamespace(**deepcopy(cfg_re50)))
        elif name == 'mobilenet0.25':
            return SimpleNamespace(weights=os.path.realpath(os.path.join('pretrain_models/face_lib/face_parsing/ibug', 'mobilenet0.25_Final.pth')),
