...
"""
from https://github.com/HowieMa/CVTHead/blob/main/decalib/deca.py
"""
import os, sys
import torch
import torchvision
...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=False, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'].cpu(), expression_params=codedict['exp'].cpu(), pose_params=codedict['pose'].cpu())
        ...
        # cam: {s,tx,ty}
        # [X, Y, Z] -> s * (X+tx, (-Y+ty), -Z)
        landmarks2d = util.batch_orth_proj(landmarks2d, codedict['cam'].cpu())[:,:,:2]    # (B, 68, 2)
        landmarks2d[:,:,1:] = -landmarks2d[:,:,1:]

        landmarks3d = util.batch_orth_proj(landmarks3d, codedict['cam'].cpu());
        landmarks3d[:,:,1:] = -landmarks3d[:,:,1:] 

        trans_verts = util.batch_orth_proj(verts, codedict['cam'].cpu());
