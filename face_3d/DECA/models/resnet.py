...
def load_ResNet50Model():
    ...
    copy_parameter_from_resnet(model, torchvision.models.resnet50(weights = False).state_dict())
    return model

def load_ResNet101Model():
    model = ResNet(Bottleneck, [3, 4, 23, 3])
    copy_parameter_from_resnet(model, torchvision.models.resnet101(weights = True).state_dict())
    return model

def load_ResNet152Model():
    model = ResNet(Bottleneck, [3, 8, 36, 3])
    copy_parameter_from_resnet(model, torchvision.models.resnet152(weights = True).state_dict())
    return model
